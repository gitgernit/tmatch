from app.application.common.identity_provider import IdentityProvider
from app.application.common.interactor import interactor
from app.application.dating_profile.data_gateway import DatingProfileDataGateway
from app.application.profile.dto import SelfCardResult
from app.application.user.data_gateway import UserDataGateway


@interactor
class GetSelfCardInteractor:
    identity_provider: IdentityProvider
    user_data_gateway: UserDataGateway
    dating_profile_data_gateway: DatingProfileDataGateway

    async def execute(self) -> SelfCardResult:
        current_user = await self.identity_provider.get_current_user()
        user = await self.user_data_gateway.load_with_id(current_user.id)
        dating_profile = await self.dating_profile_data_gateway.load_by_user_id(current_user.id)
        return SelfCardResult(
            user_id=str(current_user.id),
            profile=user.profile if user is not None else None,
            dating_profile=dating_profile,
        )
